...
class SolanaTools:
    def __init__(self, rpc_url: str, private_key: Optional[str] = None, session_id: Optional[str] = None) -> None:
        ...
        if private_key:
            ...
            logger.info("Initialized Solana tools without wallet - will use session-based keys if available")

    def _initialize_keypair(self, private_key: str) -> None:
        """Initialize keypair from private key string."""
        try:
    ...
    async def set_session_private_key(self, private_key: str) -> bool:
        """
        Set private key for the current session.
        This allows the AI to request private keys from the frontend chat.
        """
        try:
            self._initialize_keypair(private_key)
            logger.info(f"Session {self.session_id} private key set successfully")
            return True
        except Exception as e:
            logger.error(f"Failed to set session private key: {e}")
            return False
    ...
    async def transfer_sol(self, to_address: str, amount: float) -> Dict[str, Any]:
        ...
        if not self.keypair or not self.wallet_address:
            return {
                ...
                "message": "Please provide your private key to perform transactions. Use the request_private_key tool first."
            }
