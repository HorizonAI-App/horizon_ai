...
class AgentBuilder:
    ...
    async def build(self, context: Optional[RequestContext] = None, session_id: Optional[str] = None) -> SAMAgent:
        ...
        # For web app: Don't use environment private key, rely on session-based keys
        if session_id:
            # Session-based private key will be set via frontend_auth tools
            logger.info(f"Building agent for session {session_id} - private key will be requested from frontend")
        else:
            # CLI mode: Use environment or secure storage
